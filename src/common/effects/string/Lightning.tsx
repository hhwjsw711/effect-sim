...
export function Lightning({
  ...
  useEffectFrame((frame) => {
    ...
    if (Math.random() < props.strikeChance) {
      const centerPos = Math.random() * ledCount;
      const intensity = 0.7 + Math.random() * 0.3;
      const spread =
        props.minSpread + Math.random() * (props.maxSpread - props.minSpread);
    ...
    boltsRef.current = boltsRef.current.filter((bolt) => {
      ...
      for (let s = 0; s < numStrikes; s++) {
        const strikeCenter =
          bolt.centerPosition + (Math.random() - 0.5) * bolt.spread * 0.3;
