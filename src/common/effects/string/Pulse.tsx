...
export function Pulse({
  ...
  useEffectFrame(() => {
    ...
    const r = Math.round(
      props.fromColor[0] + (props.toColor[0] - props.fromColor[0]) * pulseRatio,
    );
    const g = Math.round(
      props.fromColor[1] + (props.toColor[1] - props.fromColor[1]) * pulseRatio,
    );
    const b = Math.round(
      props.fromColor[2] + (props.toColor[2] - props.fromColor[2]) * pulseRatio,
    );
